...
- None of this storage systems is guaranteed to work on paper/spigot/folia/whatever modified server. If you're playing on a public server that doesn't allow chunkloaders or periodically clears item entities, then said server is also very likely to disrupt normal redstone behaviour in the disingenuous attempt of reducing lag, and there's nothing we can do about that.
